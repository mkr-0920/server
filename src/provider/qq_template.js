...
const track = async (id) => {
	// Robustly extract the mid (song property) regardless of whether id is an object or a string
	const mid = (typeof id === 'object') ? id.song : id;
	if (!mid) return Promise.reject();

	try {
		// The user's local API expects the mid to be passed as ?mid=
		const apiUrl = `http://127.0.0.1:5000/api/qq?mid=${mid}`;
        // If you also need to pass X-API-Key, uncomment and add it to the request headers below
        const apiHeaders = {};
